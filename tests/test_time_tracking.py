...
async def test_time_tracking_segments(client, db_session):
    ...
    segments = await db_session.scalars(select(TicketTimeSegment).where(TicketTimeSegment.ticket_id == ticket_id))
